...
- **Cursor** (🆕 27-ago, evaluado, no implementado, **2 caminos posibles, decisión pendiente del usuario**):
  - **Camino A — Bugbot vía GitHub.** Confirmado que `archicheck` es público en GitHub (`TuQRApp/archicheck`) → probablemente entra en el tier gratuito de Bugbot para repos públicos/uso individual; `archicheck-worker` da 404 sin autenticación → parece privado → ahí Bugbot se cobra por corrida (~US$1-1.50 c/u desde jun-2026) más asiento de Cursor (US$20-40/usuario). **Requiere cambiar de flujo**: hoy se comitea directo a `main` en ambos repos (nunca se abrió un PR) — Bugbot solo revisa Pull Requests, no commits sueltos, así que hace falta trabajar en una rama + abrir PR para lo que se quiera revisar. El alcance de la revisión lo define el contenido del PR (control total de qué se revisa). Requiere además que el usuario instale la GitHub App de Cursor en el repo (acción de cuenta de GitHub, Claude no puede hacerla). Automatizar la apertura de PRs necesitaría el CLI `gh` — **no está instalado en esta máquina todavía**.
  - **Camino B — `cursor-agent` CLI headless.** `agent -p "prompt" --output-format json`, autenticado con `CURSOR_API_KEY` — opera como agente que lee el repo él mismo (no se le pega el contenido en el prompt como a DeepSeek/Codex). Mismo flujo manual de hoy (sin PRs), pero requiere instalar el CLI (no está instalado) y una API key nueva.
  - Ninguno implementado — falta que el usuario elija A, B, o ambos, antes de instalar/configurar nada.
