...
- **🆕 Puerta señalada por cambio de grosor de línea (grueso→fino) SIN línea central (usuario, 2026-08-30, caso real Beauchef)**: variante de dibujo donde un tramo de muro pasa a línea más fina y trae solo 2 bordes opuestos, sin una tercera línea central — se marca como puerta (sin arco ni hoja fabricados, misma regla ya definida arriba para "puerta sin gozne ni arco"). Ver la misma variante para ventanas (con línea central) en la sección Ventanas — la diferencia entre ambos casos es únicamente la presencia o ausencia de esa línea central.
...
- **🆕 Variante de dibujo: cambio de grosor de línea en vez de (o además de) un símbolo rectangular (usuario, 2026-08-30, caso real Beauchef — Camarín, muro vertical entre Camarín y Baño)**: la misma firma geométrica (2 bordes opuestos + 1 línea central) puede dibujarse con línea más FINA que el resto del muro en ese tramo, en vez de (o sin) un símbolo rectangular explícito como el de las ventanas horizontales del mismo muro. Confirmado con zoom real: el tramo de línea fina sí trae sus 3 líneas paralelas (2 bordes + 1 central), solo que a menor grosor — sigue siendo la misma regla de ventana de arriba, no una tipología nueva. **Implicancia de pipeline**: un detector que dependa de un símbolo rectangular cerrado, o que descarte líneas por ser más finas que el muro circundante, se pierde esta variante — el criterio debe seguir siendo puramente geométrico (2+1 líneas paralelas), sin importar el grosor de trazo.
- **🆕 Cambio de grosor de línea SIN línea central = puerta, no ventana (usuario, 2026-08-30)**: si el tramo de línea fina dentro de un muro trae solo 2 bordes opuestos, SIN una tercera línea central, se clasifica como **puerta** (ver sección Puertas), no ventana — la presencia o ausencia de la línea central es lo único que distingue ambos casos en esta variante de dibujo.
...
- **Codex (OpenAI)** (`Fase 2/Herramientas_CubiCasa5k/revisar_con_codex.mjs`) — mismo mecanismo, mismo prompt base que DeepSeek. Usa la Responses API (`/v1/responses`, no `/v1/chat/completions`) con el modelo Codex — API distinta a la que ya usa el Worker para GPT-4o, ver comentario en el script. **✅ Ejecutado de verdad (29-ago, 2 corridas)**, ya con `OPENAI_API_KEY` provista — encontró **4 bugs reales**, todos corregidos y comiteados: (1ª corrida, `1eca3e4`) inyección de `_colab_id` en `recintos_superficies` leía `.por_nivel` cuando la estructura real es `.tabla` (plana, igual que circulaciones/iluminación) — el loop nunca encontraba nada, rompiendo la trazabilidad recinto→Colab en esa tabla; y `indexar_normativa.mjs` no deduplicaba chunks de PRC como sí hacía con OGUC/LGUC/LEY19300/DDU, pisando registros en silencio ante colisión de código. (2ª corrida, `4fa044c`) el fix anterior de `.por_nivel`→`.tabla` había copiado el campo de búsqueda viejo (`r.nombre`) en vez del real (`r.recinto`, confirmado contra el render y `keyField` de `mergeSeccionConComparacion`). También encontró (`7c04233`, corrida separada junto con DeepSeek) que `mergeResults` comparaba `estado_global` contra `APROBADO/RECHAZADO` cuando el prompt/UI real usan `APROBABLE/RECHAZABLE` — el ranking de severidad entre modelos nunca funcionó, siempre devolvía el resultado del primer modelo sin comparar de verdad. Un fix (`1eca3e4`) fue validado de forma cruzada e independiente por DeepSeek (`ceeac17`, script nuevo `validar_fix_con_deepseek.mjs`) antes de darlo por bueno. Detalle completo con conteos/commits en el roadmap canónico, entrada 29-ago.
- **Bug adicional encontrado por DeepSeek en la misma ronda** (`7c04233`): `verificador.js` `fmt()` usaba un regex que recortaba CUALQUIER cero final de un número formateado, no solo la parte decimal — `fmt(500,0)` daba "5", `fmt(1800,0)` daba "18". Bug real y de alto impacto (datos reales de Ñuñoa como `subdivision_predial_minima_m2=500` se mostraban mal en la UI); los 27 tests no lo detectaban porque solo verifican el booleano `.cumple`, no el texto numérico mostrado.
- **Cursor** (✅ decisión tomada 29-ago, ver estado más abajo):
  - **Camino A — Bugbot vía GitHub.** Confirmado que `archicheck` es público en GitHub (`TuQRApp/archicheck`) → probablemente entra en el tier gratuito de Bugbot para repos públicos/uso individual; `archicheck-worker` da 404 sin autenticación → parece privado → ahí Bugbot se cobra por corrida (~US$1-1.50 c/u desde jun-2026) más asiento de Cursor (US$20-40/usuario). **Primera PR de prueba abierta** (`test-bugbot-codigo-muerto`, PR #1, 29-ago) — limpieza chica de código muerto + fix del nombre de modelo de Codex, `mergeable: CLEAN`, tests/lint/build verificados. **Bugbot todavía no corrió de verdad**: 3 intentos de `bugbot run` en el PR, primero "Unable to authenticate", los otros 2 "Bugbot is disabled for this repository" — falta habilitarlo en el [dashboard de Bugbot](https://www.cursor.com/dashboard/bugbot) (acción de cuenta del usuario, no delegable a Claude) antes de que la prueba se complete de verdad. El PR sigue abierto, sin mergear, a la espera de esa decisión.
  - **Camino B — `cursor-agent` CLI headless.** ❌ **Descartado por el usuario (29-ago)** — intento de instalación se cortó a mitad de camino en otra ventana, decidió no retomarlo. Nada llegó a instalarse, no queda nada que limpiar.
- **Kiuwan** (evaluado, no implementado) — no es un modelo de lenguaje, es un escáner SAST (análisis estático de seguridad/calidad) con su propio motor (Kiuwan Local Analyzer, CLI) o plataforma cloud — mecanismo completamente distinto (correr un scan y leer su reporte, no "prompt + texto"). Herramienta enterprise paga — el usuario todavía no tiene cuenta/licencia, queda pendiente de evaluar contratación antes de diseñar la integración.

**Estado**: DeepSeek y Codex ejecutados, con hallazgos reales ya corregidos en cada ronda (ver detalle arriba); Cursor con decisión tomada (solo Camino A, Bugbot pendiente de habilitarse en el dashboard antes de mergear el PR de prueba); Kiuwan en evaluación, sin cuenta todavía.
