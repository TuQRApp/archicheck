...
try:
    from cuerpo_cerrado import cuerpo_cerrado_fusiona, diagnosticar_candidatos
except ImportError:
    print('cuerpo_cerrado.py no esta en este runtime -- selecciona el archivo (Fase 2/Herramientas_CubiCasa5k/cuerpo_cerrado.py):')
...
def _detectar_leyenda_simbologia(doc):
    ...
    separada, o repetirse en varias). Heuristica: un item 're'/'qu' con
    relleno de color (un "swatch") + un bloque de texto corto en la
    MISMA fila, a la derecha, a poca distancia -- se toma ese texto como
    la etiqueta de ese color exacto.
...
def extraer_datos_vectoriales(pdf_page, zoom, mpx, crop_px=None, max_largo_trazo_m=3.0, mapeo_capas=None, mapa_estado_por_color=None):
    ...
    for _caso in _CASOS_DIAG_VISUAL:
        try:
            ...
            _archivo_diag = f'diag_contexto_{_caso["nombre"]}.png'
    ...
    try:
        ...
        _radio_ventana_px = _radio_ventana_m / mpx if mpx else 250
