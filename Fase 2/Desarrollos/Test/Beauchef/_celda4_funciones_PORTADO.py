...
try:
    from cuerpo_cerrado import (cuerpo_cerrado_fusiona, diagnosticar_candidatos, clasificar_no_muro,
                                 relleno_solido_de_contexto, reconstruir_ventanas_por_jamba,
                                 cortar_muros_por_ventanas, _normalizar_ventana_para_corte,
                                 _dividir_en_muros_por_union, _distancia, _proyeccion_eje_largo,
...
# _distancia: movida a cuerpo_cerrado.py (importada arriba).
...
# _dividir_en_muros_por_union, cortar_muros_por_ventanas, _normalizar_ventana_para_corte,
# _proyeccion_eje_largo: movidas a cuerpo_cerrado.py (importadas arriba).
    ...
    for pagina_leyenda in doc:
        ...
        for path in pagina_leyenda.get_drawings():
            ...
            if fill and len(fill) >= 3:
                ...
                if cerrado:
                    ...
                    continue
                # NUEVO (2026-09-14, caso real PdV: icono de leyenda 'Se
                # retira') -- ni relleno solido ni contorno cerrado: el
                # icono es SOLO el propio achurado, cada trazo diagonal su
                # propio path independiente de 1 item, sin ningun marco que
                # los encierre. Se agrupan por color mas abajo (fuera de
                # este loop de paths) por proximidad de bbox.
                if len(items_path) == 1 and items_path[0][0] == 'l':
                    p1, p2 = items_path[0][1], items_path[0][2]
                    color_key_suelta = tuple(round(c, 3) for c in stroke[:3])
                    _lineas_sueltas_por_color.setdefault(color_key_suelta, []).append((p1.x, p1.y, p2.x, p2.y))
        # Agrupa las lineas sueltas de achurado por color + proximidad de
        # bbox (mismo criterio de "crece el bbox, nunca cadena transitiva
        # sin limite" que ya usa _dividir_en_muros_por_union) -- un cluster
        # de 3+ trazos que cabe en LIMITE_SWATCH_PT es un icono de achurado
        # real; menos de 3 es demasiado poco para distinguirlo de cualquier
        # linea suelta random de la lamina.
        for color_key_suelta, lineas in _lineas_sueltas_por_color.items():
            clusters = []
            for (lx1, ly1, lx2, ly2) in lineas:
                bx0, bx1 = min(lx1, lx2), max(lx1, lx2)
                by0, by1 = min(ly1, ly2), max(ly1, ly2)
                asignado = False
                for c in clusters:
                    nminx, nmaxx = min(c['minx'], bx0), max(c['maxx'], bx1)
                    nminy, nmaxy = min(c['miny'], by0), max(c['maxy'], by1)
                    if (nmaxx - nminx) <= LIMITE_SWATCH_PT and (nmaxy - nminy) <= LIMITE_SWATCH_PT:
                        c['n'] += 1
                        c['minx'], c['maxx'], c['miny'], c['maxy'] = nminx, nmaxx, nminy, nmaxy
                        asignado = True
                        break
                if not asignado:
                    clusters.append({'n': 1, 'minx': bx0, 'maxx': bx1, 'miny': by0, 'maxy': by1})
            for c in clusters:
                if c['n'] < 3:
                    continue
                swatches.append({'fill': color_key_suelta, 'rect': (c['minx'], c['miny'], c['maxx'], c['maxy'])})
        if not swatches:
            continue
        lineas_texto = []
        anclas_simbologia = []
        for block in pagina_leyenda.get_text('dict').get('blocks', []):
            for line in block.get('lines', []):
                ...
                if texto_linea:
                    lineas_texto.append({'texto': texto_linea, 'bbox': line['bbox'], 'dir': line.get('dir', (1.0, 0.0))})
                    if texto_linea.strip().upper() == 'SIMBOLOGIA':
                        bx0, by0, bx1, by1 = line['bbox']
                        anclas_simbologia.append(((bx0 + bx1) / 2, (by0 + by1) / 2))
        if anclas_simbologia:
            # NUEVO (2026-09-14, caso real PdV): una lamina de plano tiene
            # MUCHOS elementos achurados que no son la leyenda (ej. la
            # numeracion de una escalera, tambien dibujada con achurado,
            # resulto ser exactamente el mismo color amarillo que el
            # achurado 'Se retira' de la leyenda real -- confirmado con
            # datos reales, robaba el match). El color NUNCA decide si algo
            # es geometria real (regla del arquitecto, ver docstring), pero
            # tampoco alcanza para decidir SOLO por color que un swatch es
            # DE LA LEYENDA -- se acota la busqueda de swatches a una
            # vecindad real del texto 'SIMBOLOGIA' de esa misma pagina
            # (la leyenda vive ahi, no en cualquier parte del plano).
            RADIO_LEYENDA_PT = 150

            def _cerca_de_alguna_ancla(rect):
                cx, cy = (rect[0] + rect[2]) / 2, (rect[1] + rect[3]) / 2
                return any(_distancia((cx, cy), a) <= RADIO_LEYENDA_PT for a in anclas_simbologia)

            swatches = [sw for sw in swatches if _cerca_de_alguna_ancla(sw['rect'])]
        if not swatches:
            continue
        for sw in swatches:
            x0, y0, x1, y1 = sw['rect']
            ...
            for lt in lineas_texto:
                ...
                if mejor is None or dist < mejor[0]:
                    mejor = (dist, lt['texto'])
            if mejor is None:
                # NUEVO (2026-09-14, caso real PdV: leyenda "Se retira"/"Se
                # construye" con texto rotado 90 grados en columna) -- SOLO
                # se prueba si el caso horizontal de arriba no encontro nada,
                # para no robarle un match correcto a un swatch horizontal
                # (confirmado con datos reales: probarlo siempre, no solo
                # como fallback, generaba falsos positivos en swatches que ya
                # emparejaban bien por el criterio horizontal). Mismo
                # criterio, rotado: swatch y texto comparten COLUMNA (rango
                # en X superpuesto, no fila), texto ARRIBA o ABAJO del
                # swatch (no a la derecha).
                for lt in lineas_texto:
                    lx0, ly0, lx1, ly1 = lt['bbox']
                    dx, dy = lt['dir']
                    if abs(dy) <= abs(dx):
                        continue  # no es texto vertical, ya se probo arriba
                    lcx = (lx0 + lx1) / 2
                    if abs(lcx - cx) > ancho_sw + 3:
                        continue  # no esta en la misma columna que el swatch
                    if ly0 >= y1 - 2:
                        dist = ly0 - y1  # texto abajo del swatch
                    elif ly1 <= y0 + 2:
                        dist = y0 - ly1  # texto arriba del swatch
                    else:
                        continue  # se superpone en Y con el swatch, no es un layout arriba/abajo limpio
                    if dist > TOL_CERCANIA_TEXTO_PT:
                        continue
                    if mejor is None or dist < mejor[0]:
                        mejor = (dist, lt['texto'])
...
def extraer_datos_vectoriales(pdf_page, zoom, mpx, crop_px=None, max_largo_trazo_m=3.0, mapeo_capas=None, mapa_estado_por_color=None):
    ...
    # ── Paso 1.35 (NUEVO 2026-09-14, D-muro-curvo, caso real PdV MU01,
    #    esquina redondeada Terraza/Cocina) -- detecta muros CURVOS desde las
    #    curvas Bezier de 'otros_items'. Hasta ahora esas curvas SOLO se
    #    usaban para el clasificador de arco de puerta (mas abajo, Paso 6).
    #    Firma real confirmada con datos (PdV pag1): 2 o 3 curvas
    #    CONCENTRICAS (mismo centro real, radios distintos, con espesor
    #    plausible entre el radio mayor y el menor) -- el mismo patron D1-D3
    #    de "2 bordes + 1 linea central" que ya se usa para muros/ventanas
    #    RECTAS, pero curvo. Se distingue de un arco de puerta (curva unica,
    #    sin pareja concentrica) por tener esa pareja -- nunca por el barrido
    #    angular ni el radio (un muro curvo puede tener cualquier barrido).
    #
    #    Metodo: un arco suave se exporta como VARIOS fragmentos Bezier
    #    cortos encadenados (confirmado real: el arco de MU01 son 3
    #    fragmentos por cara, con el ancho de trazo alternando entre
    #    fragmentos de la MISMA cara -- no sirve para distinguir caras). Se
    #    encadenan por extremo compartido en curvas completas, se ajusta un
    #    circulo a cada una (mismo metodo Kasa que el clasificador de
    #    puerta), se agrupan las que comparten centro real, y el radio mayor
    #    y menor del grupo son las 2 caras reales -- cualquier radio
    #    intermedio es la linea central, se descarta igual que un D1-D3
    #    recto. Los conectores rectos entre las 2 caras (ya existen en
    #    segmentos_l como 'l' normales, ver mas abajo) se marcan por
    #    CERCANIA A LA CIRCUNFERENCIA de cada cara, no por coincidir con un
    #    extremo puntual de bezier -- confirmado real que un conector cae
    #    sobre un empalme INTERMEDIO entre 2 fragmentos, no en el extremo
    #    final de la curva completa.
    _TOL_RESIDUAL_MURO_CURVO = 0.15  # que tan circular debe ser el ajuste (mismo umbral que el clasificador de puerta, TOL_ARCO_RESIDUAL_REL, definido mas abajo -- no se puede reusar esa constante aca, se define despues en la funcion)
    _TOL_ESPESOR_MURO_CURVO_MIN_M = _param('D1-ancho-emparejamiento', 'tol_min_m', 0.08)
    _TOL_ESPESOR_MURO_CURVO_MAX_M = _param('D1-ancho-emparejamiento', 'tol_max_m', 0.9)
    _TOL_CENTRO_CURVA_PX = 6  # 2 curvas son "concentricas" si sus centros ajustados caen dentro de esto -- ruido de trazado CAD, no escala de muro
    _TOL_CADENA_BEZIER_PX = 3  # 2 fragmentos Bezier son "la misma curva" si comparten extremo dentro de esto
    _TOL_CONECTOR_CURVO_PX = 6  # un extremo de conector recto "cae sobre" la circunferencia de una cara si su distancia al radio ajustado es menor a esto

    def _muestrear_bezier_muro_curvo(p0, p1, p2, p3, n=12):
        _pts = []
        for _i in range(n):
            _t = _i / (n - 1)
            _mt = 1 - _t
            _x = _mt**3*p0[0] + 3*_mt**2*_t*p1[0] + 3*_mt*_t**2*p2[0] + _t**3*p3[0]
            _y = _mt**3*p0[1] + 3*_mt**2*_t*p1[1] + 3*_mt*_t**2*p2[1] + _t**3*p3[1]
            _pts.append((_x, _y))
        return _pts

    def _ajustar_circulo_muro_curvo(puntos):
        if len(puntos) < 4:
            return None
        _xs = np.array([p[0] for p in puntos], dtype=float)
        _ys = np.array([p[1] for p in puntos], dtype=float)
        _A = np.column_stack([2 * _xs, 2 * _ys, np.ones(len(_xs))])
        _b = _xs ** 2 + _ys ** 2
        try:
            _sol, *_r_lsq = np.linalg.lstsq(_A, _b, rcond=None)
        except Exception:
            return None
        _cx, _cy, _c = _sol
        _r2 = _c + _cx ** 2 + _cy ** 2
        if _r2 <= 0:
            return None
        _r = math.sqrt(_r2)
        _dists = np.sqrt((_xs - _cx) ** 2 + (_ys - _cy) ** 2)
        _residual = float(np.std(_dists) / _r) if _r > 0 else 1.0
        return _cx, _cy, _r, _residual

    _curvas_bezier_raw = [pts for (_op_raw, pts, _ancho_raw) in otros_items if _op_raw == 'c' and len(pts) == 4]
    _piezas_restantes = list(range(len(_curvas_bezier_raw)))
    _cadenas_bezier = []
    while _piezas_restantes:
        _cadena = [_piezas_restantes.pop(0)]
        _cambiado = True
        while _cambiado:
            _cambiado = False
            _p0_cadena = _curvas_bezier_raw[_cadena[0]][0]
            _p3_cadena = _curvas_bezier_raw[_cadena[-1]][3]
            for _k, _idx in enumerate(_piezas_restantes):
                _pts_k = _curvas_bezier_raw[_idx]
                if _distancia(_p3_cadena, _pts_k[0]) <= _TOL_CADENA_BEZIER_PX:
                    _cadena.append(_piezas_restantes.pop(_k))
                    _cambiado = True
                    break
                if _distancia(_p0_cadena, _pts_k[3]) <= _TOL_CADENA_BEZIER_PX:
                    _cadena.insert(0, _piezas_restantes.pop(_k))
                    _cambiado = True
                    break
        _cadenas_bezier.append(_cadena)

    _curvas_ajustadas = []
    for _cadena in _cadenas_bezier:
        _muestras_cadena = []
        for _idx in _cadena:
            _muestras_cadena.extend(_muestrear_bezier_muro_curvo(*_curvas_bezier_raw[_idx]))
        _ajuste = _ajustar_circulo_muro_curvo(_muestras_cadena)
        if _ajuste is None:
            continue
        _cx_c, _cy_c, _r_c, _res_c = _ajuste
        if _res_c > _TOL_RESIDUAL_MURO_CURVO:
            continue
        _curvas_ajustadas.append({'muestras': _muestras_cadena, 'cx': _cx_c, 'cy': _cy_c, 'radio_px': _r_c})

    # NUEVO (2026-09-14, correccion real -- caso PdV MU01): agrupar SOLO por
    # centro compartido no alcanza. Confirmado con datos reales: el centro
    # de MU01 (1352.8, 2392.x) es compartido por 7 curvas distintas, no 3 --
    # varias son elementos curvos DISTINTOS del plano (otro arco/canopy mas
    # lejano) que por coincidencia de diseño comparten el mismo punto de
    # centro. Tomar el radio minimo/maximo global del grupo entero mezclaba
    # una cara real con un elemento sin relacion. La señal real de "son la
    # misma pared" no es solo centro+radio-plausible: es que ADEMAS cubran
    # el MISMO RANGO ANGULAR (se superponen en el arco, no solo en el punto
    # de centro) -- se empareja curva por curva, no grupo completo.
    def _angulos_desenvueltos_muro_curvo(muestras, cx, cy):
        _angs = []
        _prev = None
        _total = 0.0
        for _p in muestras:
            _a = math.atan2(_p[1] - cy, _p[0] - cx)
    ...
    for _curva in _curvas_ajustadas:
        _angs = _angulos_desenvueltos_muro_curvo(_curva['muestras'], _curva['cx'], _curva['cy'])
        _curva['angulos'] = _angs
        _curva['ang_min'] = min(_angs)
        _curva['ang_max'] = max(_angs)

    _grupos_concentricos = []
    for _curva in _curvas_ajustadas:
        _asignado = False
        for _grupo_c in _grupos_concentricos:
            _rep = _grupo_c[0]
            if _distancia((_curva['cx'], _curva['cy']), (_rep['cx'], _rep['cy'])) <= _TOL_CENTRO_CURVA_PX:
                _grupo_c.append(_curva)
                _asignado = True
                break
        if not _asignado:
            _grupos_concentricos.append([_curva])

    # NUEVO (correccion 4, mismo caso real -- columnas de Beauchef): un
    # circulo completo (pilar/columna redonda) a veces se exporta como 2+
    # arcos separados del MISMO radio (confirmado real: 2 mitades de
    # exactos 180 grados cada una, ninguna individualmente cerca de los
    # 300 grados del filtro de arriba, pero juntas suman 360). Se detecta
    # agrupando las curvas de un mismo grupo concentrico por NIVEL DE
    # RADIO (radios casi iguales entre si -- posibles fragmentos del mismo
    # circulo, no necesariamente concentricos a proposito con otro radio
    # distinto) y sumando su barrido angular combinado -- si se acerca a
    # 360 grados, TODAS las curvas de ese nivel son parte de un circulo
    # cerrado, no de un muro curvo (que es una esquina/tramo, nunca un
    # circulo completo).
    _TOL_BARRIDO_MAX_MURO_CURVO_RAD = math.radians(300)  # un arco de MURO real es una esquina/tramo, no un circulo cerrado -- >= 300 grados es un pilar/columna redonda (confirmado real: columnas de Beauchef, 2 mitades de ~180 grados cada una compartiendo centro y espesor plausible)
    _TOL_RADIO_MISMO_NIVEL_PX = 3
    for _grupo_c in _grupos_concentricos:
        _niveles_radio = []
        for _curva in _grupo_c:
            _asignado_nivel = False
            for _nivel in _niveles_radio:
                if abs(_curva['radio_px'] - _nivel['radio_px']) <= _TOL_RADIO_MISMO_NIVEL_PX:
                    _nivel['curvas'].append(_curva)
                    _asignado_nivel = True
                    break
            if not _asignado_nivel:
                _niveles_radio.append({'radio_px': _curva['radio_px'], 'curvas': [_curva]})
        for _nivel in _niveles_radio:
            if len(_nivel['curvas']) < 2:
                continue
            _cobertura_total = sum(c['ang_max'] - c['ang_min'] for c in _nivel['curvas'])
            if _cobertura_total >= _TOL_BARRIDO_MAX_MURO_CURVO_RAD:
                for _c in _nivel['curvas']:
                    _c['es_circulo_completo'] = True

    # NUEVO (correccion 2, mismo caso real): la fraccion de solape se mide
    # contra el arco MAS GRANDE de cada par, no el mas chico -- confirmado
    # real que usar el mas chico deja pasar cruces espurios entre una curva
    # angosta (linea central real, o un elemento sin relacion) y una curva
    # mucho mas ancha que simplemente la "contiene" de pasada (ej. la cara
    # exterior real de MU01, span ancho, terminaba emparejando ademas con
    # la linea central Y con un elemento curvo totalmente distinto y mas
    # lejano que compartia el mismo centro de diseño por coincidencia).
    # Contra el arco mas grande, esos cruces asimetricos (uno mucho mas
    # angosto que el otro) quedan fuera solos, sin necesidad de reglas
    # ad-hoc por caso -- 2 caras reales de la MISMA pared curva deben
    # cubrir basicamente el mismo tramo angular, no solo que una quepa
    # dentro de la otra.
    _TOL_SOLAPE_ANGULAR_FRAC = 0.6  # fraccion minima (del arco MAS GRANDE del par) que 2 curvas deben compartir en angulo para contar como la misma pared
    _pares_muro_curvo = []
    for _grupo_c in _grupos_concentricos:
        if len(_grupo_c) < 2:
            continue  # curva sin pareja concentrica -- arco de puerta o linea de referencia, no muro curvo
        for _ia in range(len(_grupo_c)):
            for _ib in range(_ia + 1, len(_grupo_c)):
                _ca, _cb = _grupo_c[_ia], _grupo_c[_ib]
                if _ca.get('es_circulo_completo') or _cb.get('es_circulo_completo'):
                    continue  # parte de un circulo completo (pilar/columna, o combinacion de fragmentos que suman 360 grados) -- no un muro curvo real
                if (_ca['ang_max'] - _ca['ang_min']) >= _TOL_BARRIDO_MAX_MURO_CURVO_RAD or (_cb['ang_max'] - _cb['ang_min']) >= _TOL_BARRIDO_MAX_MURO_CURVO_RAD:
                    continue  # barrido casi de circulo completo -- pilar/columna redonda u otro objeto circular (sanitario, mueble), no un muro curvo real
                _espesor_m = abs(_ca['radio_px'] - _cb['radio_px']) * mpx
                if not (_TOL_ESPESOR_MURO_CURVO_MIN_M <= _espesor_m <= _TOL_ESPESOR_MURO_CURVO_MAX_M):
                    continue
                _lo = max(_ca['ang_min'], _cb['ang_min'])
                _hi = min(_ca['ang_max'], _cb['ang_max'])
                _solape = _hi - _lo
                if _solape <= 0:
                    continue
                _span_a = _ca['ang_max'] - _ca['ang_min']
                _span_b = _cb['ang_max'] - _cb['ang_min']
                if _span_a <= 0 or _span_b <= 0 or _solape < _TOL_SOLAPE_ANGULAR_FRAC * max(_span_a, _span_b):
                    continue  # comparten centro por coincidencia, pero no cubren el mismo tramo del plano -- no es la misma pared
                _pares_muro_curvo.append((_ca, _cb, _lo, _hi))

    import os as _os_dbg
    if _os_dbg.environ.get('DEBUG_MURO_CURVO'):
        for _gi, _gc in enumerate(_grupos_concentricos):
            if len(_gc) >= 2:
                print(f'  DEBUG grupo {_gi}: {len(_gc)} curva(s) -- ' +
                      ', '.join(f"r={c['radio_px']:.1f}px c=({c['cx']:.1f},{c['cy']:.1f}) ang=[{c['ang_min']:.2f},{c['ang_max']:.2f}] ext=[{tuple(round(v) for v in c['muestras'][0])}..{tuple(round(v) for v in c['muestras'][-1])}]" for c in _gc))
        print(f'  DEBUG pares validados (centro+espesor+solape angular): {len(_pares_muro_curvo)}')
        for _ca, _cb, _lo, _hi in _pares_muro_curvo:
            print(f'    par: rA={_ca["radio_px"]:.1f} [{_ca["ang_min"]:.2f},{_ca["ang_max"]:.2f}] <-> rB={_cb["radio_px"]:.1f} [{_cb["ang_min"]:.2f},{_cb["ang_max"]:.2f}] solape=[{_lo:.2f},{_hi:.2f}]')

    _segmentos_muro_curvo_agregados = 0
    _n_conectores_muro_curvo = 0
    _n_grupos_muro_curvo_validos = len(_pares_muro_curvo)
    for _ca, _cb, _lo, _hi in _pares_muro_curvo:
        # NUEVO (correccion 3, mismo caso real): exportar el rango COMPLETO
        # de cada curva ya validada, no solo la porcion superpuesta con su
        # pareja -- confirmado real que recortar a la interseccion cortaba
        # la punta de la cara interior justo donde empalma con el muro
        # recto vecino (MU01 quedaba sin conectar). Con el criterio de
        # solape ya corregido (contra el arco MAS GRANDE, ver arriba) cada
        # curva aparece en una sola pareja real -- exportar el rango
        # completo ya no reintroduce el cruce espurio que motivo el recorte.
        for _cara in (_ca, _cb):
            _muestras_rango = _cara['muestras']
            for _i in range(len(_muestras_rango) - 1):
                segmentos_l.append({
                    'p1': _muestras_rango[_i], 'p2': _muestras_rango[_i + 1],
                    'ancho_linea': 0, 'color': None, 'fill': None, 'layer': None, 'dashes': None,
                    'es_muro_curvo': True,
                })
                _segmentos_muro_curvo_agregados += 1
        for _s in segmentos_l:
            if _s.get('es_muro_curvo'):
                continue
            _d1_int = abs(_distancia(_s['p1'], (_ca['cx'], _ca['cy'])) - _ca['radio_px'])
            _d1_ext = abs(_distancia(_s['p1'], (_cb['cx'], _cb['cy'])) - _cb['radio_px'])
            _d2_int = abs(_distancia(_s['p2'], (_ca['cx'], _ca['cy'])) - _ca['radio_px'])
            _d2_ext = abs(_distancia(_s['p2'], (_cb['cx'], _cb['cy'])) - _cb['radio_px'])
            _p1_interior = _d1_int <= _TOL_CONECTOR_CURVO_PX
            _p1_exterior = _d1_ext <= _TOL_CONECTOR_CURVO_PX
            _p2_interior = _d2_int <= _TOL_CONECTOR_CURVO_PX
            _p2_exterior = _d2_ext <= _TOL_CONECTOR_CURVO_PX
            if (_p1_interior and _p2_exterior) or (_p1_exterior and _p2_interior):
                _s['es_muro_curvo'] = True
                _n_conectores_muro_curvo += 1
    if _segmentos_muro_curvo_agregados:
        print(f'  ✓ MURO CURVO (D1-muro-curvo): {_n_grupos_muro_curvo_validos} pareja(s) de curvas concentricas validada(s) '
              f'(centro + espesor plausible + solape angular >= {_TOL_SOLAPE_ANGULAR_FRAC:.0%}), {_segmentos_muro_curvo_agregados} segmento(s) '
              f'de cara curva agregados a segmentos_l, {_n_conectores_muro_curvo} conector(es) recto(s) marcados -- ver catalogo_tipologias.py')
    ...
    # NUEVO (2026-09-05, GAP-GEO-VENT-001 -- fix acotado, ver Diseno_Funcional
    # §2.9 y catalogo_tipologias.py D1-D3-ventana-lineas-centrales): clasificar
    # ventana/hoja-vano ANTES de armar muros_geo, no despues como corria hasta
    # ahora (ahi abajo solo servia para colorear el PNG de diagnostico).
    # clasificar_no_muro() identifica segmentos por id() de objeto en memoria
    # -- una vez que el loop de export de mas abajo arma 'segmentos_muro' con
    # dicts NUEVOS (coordenadas ya ajustadas), esa identidad se pierde para
    # siempre y no hay como cruzarla de vuelta contra un MU## ya exportado.
    # Por eso el fix nunca se conecto antes: no faltaba una linea, faltaba
    # mover CUANDO se calcula. Mover el calculo aca no cambia el resultado
    # (depende solo de 'protegido', que no cambia entre aca y donde corria
    # antes, linea ~2038 en la version vieja) -- solo convierte esos ids de
    # memoria a INDICES estables de segmentos_l, el unico identificador que
    # sobrevive a la transformacion de coordenadas del loop de abajo.
    _seg_pool_clasif = []
    _indice_por_id_seg = {}
    for _i in range(len(segmentos_l)):
        if protegido[_i]:
            _seg_pool_clasif.append(segmentos_l[_i])
            _indice_por_id_seg[id(segmentos_l[_i])] = _i
    ...
    _ventana_indices = {_indice_por_id_seg[_sid] for _sid in _ventana_ids}
    print(f'  ✓ Clasificacion ventana/hoja-vano (D1-D3, adelantada antes del export de muros -- GAP-GEO-VENT-001): '
          f'ventana={len(_ventana_indices)} segs, hoja/vano confirmada={len(_hoja_ids)} segs, '
          f'hoja/vano duda={len(_hoja_duda_ids)} segs, conflictos={len(_conflicto_ids)}')
    ...
    muros_geo = []
    puertas_geo = []
    # NUEVO (2026-09-05, GAP-GEO-VENT-001): ventanas cuya linea central quedo
    # aislada como su propio grupo de conectividad chico -- ver mas abajo,
    # dentro del loop de cadenas -- se exportan aca, nunca a muros_geo. Caso
    # real confirmado: MU02, MU14, MU15, MU18-21, MU23-26 (Beauchef, 21-31 ago).
    ventanas_simples_por_linea_central = []
    ...
    # arquitecto: el color NUNCA decide POR SI SOLO si algo es geometria real,
    # eso lo decide la forma. El color solo se usa para ETIQUETAR el resultado
    # como agregado/eliminado, leyendo la leyenda SIMBOLOGIA real del documento
    # (_detectar_leyenda_simbologia, corrida una vez fuera de esta funcion) --
    # nunca hardcodeado por proyecto.
    #
    # MATIZ (2026-09-14, caso real PdV MU33): el achurado real de una zona "Se
    # retira" es un patron de lineas DIAGONALES (firma D.achurado, confirmado
    # con datos reales: 71% de los trazos de ese color a ~45 grados) -- eso SI
    # es forma, no color. Pero el CAD dibuja tambien el marco/contorno de esa
    # zona (lineas rectas a 0/90) con el mismo color que el achurado -- ese
    # marco recto pasaba el filtro angular de abajo como candidato a muro real
    # y terminaba fusionandose con un muro real vecino solo por tocarlo (ver
    # cuerpo_cerrado_fusiona, prioridad de contacto directo). Instruccion
    # explicita del arquitecto: excluir esas rectas -- SOLO el patron diagonal
    # indica achurado real, una recta del mismo color no prueba nada por si
    # sola. Se detecta el achurado por FORMA (3+ trazos diagonales de un mismo
    # color, cerca entre si) y RECIEN AHI se usa el color para excluir las
    # rectas vecinas de ese mismo color -- sigue siendo "la forma decide",
    # ahora en dos pasos en vez de uno.
    def _es_color_por_defecto(color_key):
        # NUEVO (2026-09-14, caso real Beauchef): negro/blanco no son un color
        # DISTINTIVO -- son el color por defecto de casi cualquier trazo en un
        # plano monocromo (confirmado real: 7345 trazos diagonales en negro en
        # una sola pagina de Beauchef, achurado de material comun y corriente
        # sin ningun significado especial). Un achurado con significado real
        # (Se retira/Se construye) SIEMPRE se dibuja en un color distintivo a
        # proposito, para que un humano lo distinga a simple vista de la
        # geometria normal -- esa es la señal real, no "comparte color con
        # ALGUN achurado en algun lugar de la pagina". Sin esta guarda, en un
        # plano monocromo CUALQUIER muro negro cerca de CUALQUIER achurado
        # negro (omnipresente) se excluiria por error.
        return all(abs(c - 0.0) < 0.03 for c in color_key) or all(abs(c - 1.0) < 0.03 for c in color_key)

    _diagonales_por_color = {}
    for _s_diag in segmentos_l:
        _color_diag = _s_diag.get('color')
        if not _color_diag or len(_color_diag) < 3:
            continue
        _color_key_diag = tuple(round(c, 2) for c in _color_diag[:3])
        if _es_color_por_defecto(_color_key_diag):
            continue
        _ang_diag = _angulo_segmento(_s_diag) % 90
        if min(_ang_diag, 90 - _ang_diag) > TOL_EJE_MURO_DEG:
            _diagonales_por_color.setdefault(_color_key_diag, []).append(_s_diag)
    TOL_MARCO_ACHURADO_PX = TOL_MURO_PX * 2  # margen generoso pero acotado: el marco de un achurado esta pegado a el, no a la distancia de un muro real no relacionado
    n_marco_achurado_excluido = 0

    def _es_marco_de_achurado(seg):
        color_seg = seg.get('color')
        if not color_seg or len(color_seg) < 3:
            return False
        candidatos = _diagonales_por_color.get(tuple(round(c, 2) for c in color_seg[:3]))
        if not candidatos or len(candidatos) < 3:
            return False  # menos de 3 trazos diagonales no es un patron de achurado real
        n_cerca = 0
        for d in candidatos:
            if (_distancia_punto_segmento(seg['p1'], d['p1'], d['p2']) <= TOL_MARCO_ACHURADO_PX or
                    _distancia_punto_segmento(seg['p2'], d['p1'], d['p2']) <= TOL_MARCO_ACHURADO_PX):
                n_cerca += 1
                if n_cerca >= 3:
                    return True
        return False
    ...
    for grupo in grupos_conectividad:
        ...
        segs_reales = []
        segs_no_muro = []  # candidatos a arco de puerta (excluidos por angulo), o marco de achurado (excluido por forma+color, ver _es_marco_de_achurado)
        for i in segs_protegidos:
            if segmentos_l[i].get('es_muro_curvo'):
                # Confirmado como muro curvo por forma (D1-muro-curvo, Paso
                # 1.35: pareja de curvas concentricas con espesor plausible,
                # o conector real entre ellas) -- se salta el filtro
                # angular a proposito, no aplica a geometria curva.
                segs_reales.append(i)
                continue
            ang = _angulo_segmento(segmentos_l[i]) % 90
            if min(ang, 90 - ang) > TOL_EJE_MURO_DEG:
                segs_no_muro.append(i)
            elif _es_marco_de_achurado(segmentos_l[i]):
                n_marco_achurado_excluido += 1
                segs_no_muro.append(i)
            else:
                segs_reales.append(i)
        ...
        _hubo_muro_real_en_grupo = False
        for _cadena in cadenas_muro:
            ...
            # NUEVO (2026-09-05, GAP-GEO-VENT-001): si TODOS los segmentos de
            # esta cadena ya fueron confirmados como ventana (firma D1-D3) por
            # clasificar_no_muro() -- bloque adelantado arriba, antes de este
            # loop -- no es un muro real: es la linea central de una ventana
            # que quedo aislada como su propio grupo de conectividad chico
            # (caso real confirmado: MU02, MU14, MU15, MU18-21, MU23-26,
            # Beauchef, 21/31-ago). Se exporta aparte, nunca se descarta en
            # silencio. Si la cadena MEZCLA segmentos ventana y no-ventana, se
            # avisa pero se exporta igual como muro -- partir una cadena
            # conectada sin un criterio geometrico propio para el corte
            # arriesga mas de lo que resuelve.
            if _cadena and all(i in _ventana_indices for i in _cadena):
                ventanas_simples_por_linea_central.append({
                    'id': f'VS{len(ventanas_simples_por_linea_central) + 1:02d}',
                ...
                continue
            _n_ventana_parcial = sum(1 for i in _cadena if i in _ventana_indices)
            if _n_ventana_parcial:
                print(f'  ⚠ Cadena con {_n_ventana_parcial}/{len(_cadena)} segmentos clasificados como ventana pero no todos '
                      f'-- se exporta igual como muro (no se parte una cadena conectada sin criterio geometrico propio)')
        ...
        if segs_no_muro:
            ...
            for _sub_grupo in _sub_grupos:
                ...
                # NUEVO (2026-09-05, GAP-GEO-VENT-001): 'muros_geo[-1]' asume que
                # este grupo aporto al menos un muro real -- ya no es garantia
                # desde que las cadenas 100% ventana se desvian arriba. Guardia
                # explicita en vez de asociar por error a un muro de OTRO grupo
                # (nunca en silencio).
                if not _hubo_muro_real_en_grupo:
                    print(f'  ⚠ Puerta candidata sin muro real en su propio grupo (todas las cadenas del grupo eran ventana) '
                          f'-- muro_asociado_id queda en None, no se adivina de otro grupo')
                puertas_geo.append({
                    'id': f'PG{len(puertas_geo) + 1:02d}',
                    'segmentos': _segmentos_puerta,
                    'ancho_estimado_m': round(_radio_px * mpx, 2),
                    'muro_asociado_id': muros_geo[-1]['id'] if _hubo_muro_real_en_grupo else None,
    ...
    print(f'  ✓ Muros exportados: {len(muros_geo)} (de {sum(1 for g in grupos_conectividad if _span_grupo(segmentos_l, g) >= UMBRAL_MURO_PX)} grupos protegidos, {n_muro_desalineado} segmentos descartados por angulo no-ortogonal, {n_marco_achurado_excluido} segmentos rectos descartados por ser marco de un achurado real (mismo color, 3+ trazos diagonales cerca), {n_muro_con_estado} muros con estado agregado/eliminado detectado via leyenda -- el color ya NO excluye geometria por si solo, ver _detectar_leyenda_simbologia y _es_marco_de_achurado)')
    ...
    # NUEVO (2026-09-04, D3-ventana-reconstruccion-por-jamba, ver
    # catalogo_tipologias.py y Diseno_Funcional_ArchiCheck.md §2.9): una fila
    # de ventanas repetidas de una fachada cae en el MISMO patron geometrico
    # que _detectar_lineas_referencia_periodicas busca para deslinde/rasante
    # (colineales, gap acotado, span largo) -- sus 3 lineas reales (firma
    # D1-D3: 2 bordes + 1 central) quedan enterradas en muros_excluidos_por_
    # referencia en vez de llegar a muros_geo como fragmentos sueltos sin
    # sentido (caso real: 11 fragmentos MU## con ancho_linea_prom implausible
    # en vez de 6 ventanas, Beauchef, Roadmap 2026-08-31/09-04). Reconstruye
    # agrupando por PAR DE JAMBA real (x0,x1) -- nunca por cercania de
    # fragmentos (ese metodo ya dio una agrupacion asimetrica incorrecta).
    ventanas_reconstruidas_por_jamba = reconstruir_ventanas_por_jamba(muros_excluidos_por_referencia, mpx)
    if ventanas_reconstruidas_por_jamba:
        print(f'  ✓ Ventanas reconstruidas por par de jamba (desde muros_excluidos_por_referencia): {len(ventanas_reconstruidas_por_jamba)}')
    ...
    try:
        # NUEVO (2026-09-05, GAP-GEO-VENT-001): _seg_pool_clasif/_clasif_pagina/
        # _ventana_ids/_hoja_ids/_hoja_duda_ids/_conflicto_ids ya se calcularon
        # arriba, ANTES del loop que arma muros_geo (para poder desviar las
        # cadenas 100% ventana antes de exportarlas) -- se reusan tal cual aca,
        # ya no se recalculan. Mismo resultado exacto que antes (dependen solo
        # de 'protegido', sin cambios entre ambos puntos), una sola vez.
    ...
    # el muro, no debe correr por encima'): corte geometrico real de
    # muros_geo en cada ventana confirmada (simple + reconstruida por
    # jamba). Corre al final, sobre el muros_geo YA fusionado -- ver
    # cortar_muros_por_ventanas() para el porque de este orden.
    _ventanas_para_corte = (
        [_normalizar_ventana_para_corte(v) for v in ventanas_simples_por_linea_central] +
        [_normalizar_ventana_para_corte(v) for v in ventanas_reconstruidas_por_jamba]
    )
    if _ventanas_para_corte:
        muros_geo, _mapa_corte_ventanas = cortar_muros_por_ventanas(muros_geo, _ventanas_para_corte, mpx, TOL_MURO_PX, TOL_DIAMETRO_CLUSTER_PX)
        # Si un muro que una puerta ya tenia como muro_asociado_id se corta
        # en 2+ pedazos (o desaparece del todo), esa referencia queda
        # apuntando a un id que ya no existe -- reasociar_puertas_tras_corte
        # la repara por proximidad geometrica real (nunca por un umbral
        # arbitrario), o la deja en None si no queda ningun pedazo real.
        reasociar_puertas_tras_corte(puertas_geo, muros_geo, _mapa_corte_ventanas, mpx)
    ...
    return {
        ...
        'ventanas_reconstruidas_por_jamba': ventanas_reconstruidas_por_jamba,
        'ventanas_simples_por_linea_central': ventanas_simples_por_linea_central,
