...
| Ancho de puerta ≥ 0.80 m ("ancho libre", OGUC 4.1.7 N°6) | `piloto_ids_oguc.py`, `analizar_todos.py` | ❌ **No existe ninguna clave `'puerta'` en `OGUC_REGLAS`**, ni una regla equivalente en `reglas_verificacion.json` — pese a que el comentario de `analizar_todos.py` dice "ids y umbrales ya existentes en el proyecto, no nuevos" | **🔴 Brecha real, no consolidada** — el valor 0.80 m se verificó de forma independiente (texto OGUC vía BCN, sección 19 del roadmap BIM) pero nunca se agregó de vuelta al diccionario compartido. Riesgo concreto: si alguien corrige este umbral en un lado, el otro queda desactualizado sin que nada lo avise |
| `Pset_WallCommon.FireRating` declarado (OGUC 4.3.3) | `piloto_ids_oguc.py`, `analizar_todos.py` | ❌ Sin regla numérica/campo-checkable equivalente en `OGUC_REGLAS` ni en `reglas_verificacion.json` | **🔴 Brecha real, no consolidada** — mismo riesgo que la fila anterior |

**Pendiente de decisión del usuario, no resuelto unilateralmente acá**: si `OGUC_REGLAS`/`reglas_verificacion.json` deben extenderse para incluir puerta/FireRating como fuente única compartida entre PDF y BIM, o si se documenta formalmente una segunda fuente para BIM con el mismo criterio de auditoría que esta tabla — cualquiera de las dos opciones es aceptable en términos de este documento, lo que NO es aceptable es seguir con dos definiciones que podrían divergir sin que nada lo detecte.
...
- Sección E: decisión pendiente del usuario sobre cómo consolidar (o documentar formalmente por separado) los umbrales de puerta/FireRating.
