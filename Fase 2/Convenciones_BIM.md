...
### 🆕 Pisos y cubiertas (`IfcCovering` / `IfcSlab` / `IfcBeam` / `IfcMember`) — NO están en el catálogo de dibujo (2026-09-19)
...
- **🔴 Importante, no es solo un problema del nivel de cubierta**: `IfcCovering`/`IfcSlab`/`IfcBeam` existen en cantidades MUCHO mayores en TODOS los demás niveles del edificio (ej. "00 begane grond": 252 `IfcCovering` + 80 `IfcSlab` + 24 `IfcBeam`) — nunca se dibujan en ningún nivel, no solo en la cubierta. Es probable que esto haya sido una decisión de alcance implícita al diseñar `ESTILOS` (un piso/cubrimiento de piso dibujado a página completa taparía el resto del plano en una planta normal de muros/puertas/ventanas), pero no quedó documentada como decisión — hoy es indistinguible de un olvido. **Agregar estas 3 clases al catálogo afectaría TODOS los niveles, no solo la cubierta** — decisión de alcance pendiente del usuario, no resuelta unilateralmente: ¿agregar solo para el nivel de cubierta (caso especial), agregar en todos lados, o dejar como límite documentado?
...
- Pisos y cubiertas (`IfcCovering`/`IfcSlab`/`IfcBeam`): investigado 2026-09-19, sin implementar — decisión de alcance pendiente (¿solo cubierta, todos los niveles, o dejar como límite documentado?).
