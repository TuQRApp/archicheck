// Revisión de código periódica con Codex (OpenAI) — ver Convenciones_CAD.md D.11.
// Mismo mecanismo que revisar_con_deepseek.mjs: disparo manual, archivos completos
// (no diffs), la respuesta cruda se guarda para que Claude la filtre después.
//
// Usa la Responses API de OpenAI (no chat/completions) porque gpt-5-codex solo
// está disponible ahí. Formato de request/response distinto a DeepSeek/chat
// completions -- ver buildInput()/extraerTexto() más abajo.
//
// Uso: node revisar_con_codex.mjs
// Requiere: ../../.env.openai.local con OPENAI_API_KEY=... (raíz de archicheck/, gitignored vía *.local)
...
// Responses API: el texto de salida no viene en choices[0].message.content (eso es
// chat/completions) -- viene en un array `output` con items de tipo "message" cuyo
// `content` tiene items de tipo "output_text". Algunos SDKs exponen esto aplanado
// como `output_text`; acá lo reconstruimos a mano por si la respuesta cruda no lo trae.
...
async function revisarGrupo(grupo) {
  ...
  console.log(texto ? `  ✓ OK — guardado en ${outPath}` : `  ⚠ OK pero no se pudo extraer texto de la respuesta — revisar ${outPath} a mano`);
}
