...
#   Por cada página: Claude Vision + OpenCV + extracción vectorial + cruce
# ══════════════════════════════════════════════════════════
...
def extraer_datos_vectoriales(pdf_page, zoom, mpx, crop_px=None, max_largo_trazo_m=3.0):
    """
    NUEVO 2026-07-23 — extractor vectorial (portado desde el prototipo Node.js
    con pdfjs-dist, ver archicheck/Fase 2/Herramientas_PDF_Vectorizado/).
    ...
    Retorna:
      ...
      'n_texto', 'n_trazos', 'n_trazos_descartados_largos'
    """
    def to_px(pt):
    ...
    # ── Trazos vectoriales: muros, arcos de puerta, lineas de ventana ──
    trazos = []
...
for (PAGINA_PLANTA, ESCALA_MANUAL, crop) in entries:
    ...
    # ── 1. Claude Vision ────────────────────────────────────
    print('  → Claude Vision...')
    # CAMBIO 2026-07-23: se elimino el resize a 2000px + compresion JPEG q88.
    # Se manda la imagen completa sin comprimir (PNG, sin resize) — Anthropic
    # puede re-escalar internamente para el modelo, pero ya no sumamos
    # perdida de calidad propia encima de eso.
    ...
    # NOTA 2026-07-23: se agrego "rampas" a elementos_detectados — antes no
    # existia el campo y no era comparable contra ground truth (ver roadmap P1).
    ...
    # ── 2. OpenCV — extracción geométrica ───────────────────
    print('  → OpenCV...')
    ...
    # ── 2b. Extracción de datos vectoriales del PDF ─────────
    # NUEVO 2026-07-23. Requiere que la Celda 2 haya confirmado PDF vectorizado.
    # `doc` sigue vivo en el kernel desde la Celda 2 — no hace falta re-abrir el PDF.
    ...
    for rg in recintos_geo:
        rc     = mejor_match(rg)
        nombre = rc['nombre'] if rc else f'Espacio {rg["id"]}'
    ...
    matched = sum(1 for f in tabla if not f['nombre'].startswith('Espacio E'))
