# -*- coding: utf-8 -*-
"""Overlay de todos los muros finales de Campo Lindo pag2 (post-fix), cada
uno con un color distinto, para inspeccion visual."""
import json
import os
import sys
import colorsys
...
JSON_PATH = os.path.join(os.path.dirname(__file__), '_tmp_diag_out', '_muros_geo_clindo.json')
...
plano_full = paginas[1]  # pagina 2 (1-indexado) -> indice 1
h_f, w_f = plano_full.shape[:2]
x1, y1 = int(0.03 * w_f), int(0.02 * h_f)
x2, y2 = int(0.83 * w_f), int(0.68 * h_f)
...
img.save(os.path.join(OUT_DIR, '_overview_clindo_postfix.png'))
print('Guardado:', os.path.join(OUT_DIR, '_overview_clindo_postfix.png'))
