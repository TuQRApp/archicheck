...
**6 de los 7 P0 están remediados y verificados** (ver la columna Estado de las tablas de arriba): ACH-INFRA-001 (RAG inerte, que resultó ser 2 bugs apilados), ACH-INFRA-002 (deploy stale), ACH-INFRA-005 (`public/` faltante), ACH-CAD-002 (Celda 4 real), ACH-BIM-001 (ancho de puerta), ACH-FRONT-001 (contradicción DS50) y ACH-DATA-001 (guard de indexación). Ninguno se dio por cerrado sin verificación real: llamadas en vivo al Worker y a Supabase, datos reales de IFC, `npm run build`, `py_compile`, y el guard probado en ambos caminos.

**Queda 1 P0 sin remediar, a propósito**: ACH-WORKER-001 (CORS `*` + sin auth + sin rate limit + sin validación de payload). Es el único que no es un bug puntual sino una decisión de arquitectura — requiere definir cómo se autentica el frontend contra el Worker (token de origen, Cloudflare Access, rate limit por IP, etc.), lo que toca también el despliegue en Vercel. No se tocó sin esa definición.

**Decisiones abiertas del usuario:**
1. **ACH-WORKER-001**: ¿cómo se cierra? Es el último P0 y el de mayor superficie de riesgo económico (cualquiera con la URL consume las API keys del proyecto).
2. **Celda 4 — problema estructural**: el fix de hoy corrigió los valores, pero la línea viva sigue siendo una copia manual sin mecanismo de sincronización con `reglas_normativas.py`. Mientras siga así, el mismo desync va a volver a pasar.
3. **`indexar_normativa.mjs`**: ¿se arregla de verdad (adaptarlo al esquema `secciones` de las fuentes reales) o se deprecia formalmente y se deja a los `_idx_*.mjs` como vía única?
4. Cerrar el resto de Vercel (dominios, protección de deployment) y las routes del Worker — bajo impacto, puede esperar a la fase de infraestructura/operación.
