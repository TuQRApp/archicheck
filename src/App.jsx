...
1. EXHAUSTIVIDAD: Actúa como revisor DOM oficial. Examina sistemáticamente TODAS estas áreas normativas y genera observaciones para cada incumplimiento o punto que requiera verificación:
   ...
   • Anchos de pasillos (OGUC Art. 4.2.18: medio centímetro por persona según la carga de ocupación de la superficie servida, con un ancho mínimo de 1,10 m — el ancho de cualquier sección de vía de evacuación se determina según Art. 4.2.5, al que el propio 4.2.18 remite para pasillos que sirven a varios pisos), de rampas y accesos, y de escaleras (OGUC Art. 4.2.10, ver instrucción 3c más abajo) — mide anchos SOLO desde planta o detalle acotado; NO estimes anchos desde láminas de elevación o corte (la elevación muestra profundidad, no ancho en planta)
...
3b. Pendiente de rampas (OGUC Art. 4.1.7 N°2): la pendiente máxima NO es un valor fijo de 8%. Usa la fórmula real: i% = 12.8 - (4/7.5)*L, donde L es el largo de desarrollo de la rampa en metros (válida entre L=1.5m→12% y L=9m→8%). El texto publicado del artículo imprime la constante truncada "0,5333", pero 4/7.5 = 0,53333... es la interpolación exacta entre los 2 puntos de control que fija el propio artículo (1,5 m→12% y 9 m→8%): la truncada difiere en 0,01 puntos porcentuales en algunos largos, suficiente para cambiar un cumple/no-cumple en un caso límite, y el resto del sistema (reglas_normativas.py y el pipeline de Colab) ya calcula con la fracción exacta — usá la fracción para no contradecir las candidatas de Colab sobre la misma rampa. Calcula el máximo permitido para el largo real de la rampa antes de decidir si incumple, y cita el margen exacto en la observación (ej. "supera en 0.18 puntos porcentuales el máximo de 10.40% para un desarrollo de 4.5m" — no "supera el 8%").
...
4b. circulaciones.tabla — ancho_minimo_m: usa SIEMPRE el mínimo MÁS RESTRICTIVO de todos los artículos aplicables. Si el elemento forma parte de una ruta accesible obligatoria hacia un recinto con atención de público, el mínimo es 1.50m — cítalo como OGUC Art. 4.1.7, NUNCA con un número de artículo de DS 50/2015 (ver la instrucción IMPORTANTE sobre DS 50/2015 más arriba: ese decreto no está cargado en este sistema y no podés verificar sus números de artículo). Si citas OGUC Art. 4.2.4 con carga >50 personas, el mínimo puede superar 1.20m según fórmula. Para escaleras, aplica la tabla de la instrucción 3c (no un valor fijo). El campo ancho_minimo_m debe coincidir con el valor que determina el resultado de 'cumple'.
...
${printRef.current.innerHTML}
  ...
  async function analizar() {
    ...
    try {
      ...
      // X-ArchiCheck-Client (2026-09-21, ACH-WORKER-001): token compartido con el
      // Worker. Va solo si VITE_WORKER_TOKEN está definido en el entorno de build
      // (Vercel), y el Worker solo lo EXIGE si tiene el secret WORKER_TOKEN — así
      // se puede desplegar cada lado por separado sin romper el otro. No es
      // autenticación real (esto es una SPA pública: el token termina en el
      // bundle); es fricción contra el abuso oportunista de la URL del Worker.
      // El control que realmente acota el gasto es el rate limit del Worker.
      const H = {
        "Content-Type": "application/json",
        ...(import.meta.env.VITE_WORKER_TOKEN
          ? { "X-ArchiCheck-Client": import.meta.env.VITE_WORKER_TOKEN }
          : {}),
      };
